...
public class RESTAssuredBase extends PreAndTest{
	public static String probnum, probdescription, proburgency;
	...
	public static String getContentWithKey(Response response, String key) {
		if(response.getContentType().contains("json")) {
			JsonPath jsonPath = response.jsonPath();
			reportRequest("The "+key+" value is "+jsonPath.get(key)+" returning. ", "PASS");
			return (String) jsonPath.get(key);				

		}else {
			reportRequest("The "+key+" value is returning NULL. ", "FAIL");
